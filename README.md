...
GitHub Pages cannot apply the required HTTP anti-framing headers. The build emits
a deployment-root `_headers` file for a future header-capable static host, but
the security finding remains open until that host is provisioned, the immutable
preview passes the external-origin iframe test, and the final public response is
verified. See the [anti-framing deployment runbook](docs/anti-framing-deployment.md)
for the exact gate, cutover, rollback, and confirmation-preservation procedure.
