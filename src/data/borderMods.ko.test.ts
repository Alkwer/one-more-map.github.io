...
describe('Korean border-mod evidence', () => {
  it('keeps direct client evidence separate from confirmed numeric variants', () => {
    ...
    expect(entries.filter(([, entry]) => entry.source === 'client-screenshot')).toHaveLength(34)
    expect(
  ...
  it('derives numeric variants only from direct evidence with the same wording', () => {
    ...
    for (const [id, entry] of Object.entries(KOREAN_BORDER_MOD_EVIDENCE)) {
      ...
      const source =
        KOREAN_BORDER_MOD_EVIDENCE[derivedFrom as keyof typeof KOREAN_BORDER_MOD_EVIDENCE]
  ...
  it('preserves exact client wording and line order for the newest samples', () => {
    expect(KOREAN_BORDER_MOD_EVIDENCE['b-octoboss'].text).toBe('인접 지역들에 오물더듬이 등장')
    expect(KOREAN_BORDER_MOD_EVIDENCE['b-vaal'].text).toBe(
      '인접 지역들 내 희귀 몬스터가 바알 오브 1개를 추가로 떨어뜨림',
    )
    expect(KOREAN_BORDER_MOD_EVIDENCE['b-crabs-1'].text).toBe('인접 지역에 게 무리 8개 추가 등장')
    ...
    expect(KOREAN_BORDER_MOD_EVIDENCE['b-crabboss'].text).toBe('인접 지역에 지휘관의 파멸 등장')
    ...
    expect(KOREAN_BORDER_MOD_EVIDENCE['b-gcp']).toEqual({
      text: '인접 지역들 내 희귀 몬스터가 세공사의 프리즘 1개를 추가로 떨어뜨림',
      source: 'client-screenshot',
    })
    expect(KOREAN_BORDER_MOD_EVIDENCE['b-regret']).toEqual({
    ...
    expect(KOREAN_BORDER_MOD_EVIDENCE['b-blessed']).toEqual({
