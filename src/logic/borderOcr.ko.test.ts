...
describe('Korean border OCR matching', () => {
  ...
  it('joins the observed two-line quantity-per-connection tooltip', () => {
    const result = parseBorderOcrPayload(block(KOREAN_BORDER_MOD_EVIDENCE['b-quantconn-2'].text))
  ...
  it('tolerates the observed one-syllable Korean OCR error', () => {
    ...
    const chaosOrbs = KOREAN_BORDER_MOD_EVIDENCE['b-chaos'].text.replace('희귀', '회귀')
    const gemcuttersPrisms = KOREAN_BORDER_MOD_EVIDENCE['b-gcp'].text.replace('희귀', '회귀')
  ...
  it('does not guess an unobserved numeric tier', () => {
    const result = parseBorderOcrPayload(block('인접 지역들 내 희귀 몬스터 수 80% 증가'))
